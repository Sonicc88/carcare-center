...
@section('content')
    ...
    <div class="container-xxl rounded-2 pt-1 pb-3 mt-2" style="background-color: #FEFEFE;">
        <div class="alert alert-light p-1">
            <div class="row g-2">
                <a href="{{route('payment1')}}" class="col-md-4 col-lg-3 col-xl-2 btn btn-info fs-5 btn-sm mb-1 me-2">ກໍາລັງລ້າງ
                    <span class="badge bg-warning h5 pt-1 pb-1 pe-2 ps-2">{{$counting}}</span>
                </a>
                <a href="{{route('payment.cleaned1')}}" class="col-md-4 col-lg-3 col-xl-2 btn btn-primary fs-5 btn-sm mb-1 me-2">ລ້າງສໍາເລັດ
            ...
            <tbody>
                ...
                @foreach($que as $row)
                    <tr class="align-middle fs-6">
                        ...
                        <td>{{$row->emp_name}}</td>
                        <td>
                            <a href="{{route('payment.call1',$row->order_id)}}" class="btn btn-outline-primary">ເອີ້ນຄິວ</a>
