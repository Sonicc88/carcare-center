...
@section('content')
    <div class="alert alert-light mt-2 mb-2 pt-1 pb-1">
        <div class="row g-2">
            <a href="{{route('payment.wait')}}" class="col-sm-12 col-md-4 col-lg-3 col-xl-2 btn btn-warning btn-sm fs-5 me-2">ລໍຄິວລ້າງ
                
                <span class="badge h5 bg-danger">{{$countw}}</span>
            </a>
            <a href="{{route('payment')}}" class="col-sm-12 col-md-4 col-lg-3 col-xl-2 btn btn-info fs-5 btn-sm me-2">ກໍາລັງລ້າງ
                <span class="badge h5 bg-warning">{{$countp}}</span>
            </a>
            <a href="{{route('payment.cleaned')}}" class="col-sm-12 col-md-4 col-lg-3 col-xl-2 btn btn-primary fs-5 btn-sm me-2">ລ້າງສໍາເລັດ
                ...
                <table class="table table-light table-sm mt-3 mb-1">
                    ...
                    @if(Session::has('cart'))
                        ...
                        <tr class="align-middle text-center fs-5">
                            <td><?= $i++?></td>
                            <td><img src="{{asset($item['service_image'])}}"  width="60px" height="60px" alt="" style="border-radius: 3px;"></td>
                            <td class="text-start">{{$item['service_name']}}</td>
                            <td>{{$item['service_price']}}</td>
                            <td>
                                <form action="{{route('index.remove')}}" method="post">
                                @csrf
